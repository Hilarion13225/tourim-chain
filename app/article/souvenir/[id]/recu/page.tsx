...
export default function SouvenirRecuPage() {
  ...
  useTrackReceiptView({
    ...
    receiptType: 'SOUVENIR_ORDER_RECEIPT',
  });

  const verificationPayload = useMemo(
    () =>
      JSON.stringify({
        type: 'SOUVENIR_ORDER_RECEIPT',
        orderId,
        productId,
        productName,
        artisanName,
  ...
  function handleExportTxt() {
    ...
    anchor.href = url;
    anchor.download = `recu-souvenir-${orderId}.txt`;
    anchor.click();
    ...
    <main className="mx-auto w-full max-w-5xl space-y-6 px-6 py-8 lg:px-10">
      ...
      <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <Link
            href={`/article/souvenir/${productId}`}
            className="rounded-xl border border-zinc-300 px-4 py-2.5 text-sm font-semibold text-zinc-700"
          >
            Retour au souvenir
          </Link>
