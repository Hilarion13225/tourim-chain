...
const paymentOptions: PaymentOption[] = [
  ...
  },
  {
    ...
    account: 'Règlement à la livraison',
    minLabel: 'Min 300 F',
    maxLabel: 'Max 2 000 000 F',
  },
];

export default function SouvenirReservationPage() {
  const params = useParams<{ id: string }>();
  const router = useRouter();
  const productId = params?.id;

  const [item, setItem] = useState<SouvenirDetail | null>(null);
    ...
    async function loadData() {
      ...
      try {
        ...
        if (!authResponse.ok) {
          ...
          const returnTo = encodeURIComponent(`/article/souvenir/${productId}`);
        ...
        if (!response.ok) {
          ...
          setError(
            (data as { error?: string }).error ?? 'Souvenir introuvable',
          );
          setItem(null);
          return;
        }

        setItem(data as SouvenirDetail);
      } catch {
        if (!active) {
          return;
        }
  ...
  const imageUrl = item?.medias?.find((media) => media.type === 'IMAGE')?.url;
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setSubmitError(data.error ?? 'Achat impossible.');
        return;
      }
      ...
      const paymentParams = new URLSearchParams({
        ...
        artisanName: item.artisan?.nom ?? 'Artisan',
        fullName: nom,
        phone: telephone,
        email,
        address,
      ...
      router.push(
        `/article/souvenir/${productId}/paiement?${paymentParams.toString()}`,
      );
    } catch {
      setSubmitError('Erreur réseau pendant l’achat.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (loading) {
    return (
      <main className="mx-auto w-full max-w-7xl px-6 py-10 lg:px-10">
        <p className="text-sm text-zinc-500">Chargement de l’achat...</p>
      </main>
    );
  }
        ...
        <div className="space-y-4">
          ...
          <article className="space-y-3 rounded-2xl border border-black/10 bg-white p-5 dark:border-white/15 dark:bg-zinc-900">
            ...
            <p className="text-sm text-zinc-600 dark:text-zinc-300">
              Artisan: {item.artisan?.nom ?? 'Artisan'}
            </p>
            <p className="text-sm text-zinc-600 dark:text-zinc-300">
              Région: {item.regionOrigine ?? 'Non précisée'} • Culture:{' '}
              {item.categorie}
            </p>
            <p className="text-sm text-zinc-600 dark:text-zinc-300">
              {item.description}
            ...
            <p className="text-lg font-bold">{formatXof(unitAmount)} / unité</p>
          </article>
        </div>

        <aside className="rounded-2xl border border-black/10 bg-white p-5 dark:border-white/15 dark:bg-zinc-900">
          <h2 className="text-xl font-bold text-orange-500">Acheter</h2>
