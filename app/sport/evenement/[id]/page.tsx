...
export default function SportEvenementInscriptionPage() {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (!authResponse.ok) {
          ...
          const returnTo = encodeURIComponent(`/sport/evenement/${eventId}`);
        ...
        if (!response.ok) {
          ...
          setError(
            (data as { error?: string }).error ?? 'Événement introuvable',
          );
          setItem(null);
          return;
        }
        ...
        if (meta.itemType && meta.itemType !== 'EVENT') {
          setError('Cette ressource n’est pas un événement sportif.');
          setItem(null);
          return;
        }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setSubmitError(data.error ?? 'Inscription impossible.');
        return;
      }
      ...
      router.push(
        `/sport/evenement/${eventId}/paiement?${paymentParams.toString()}`,
      );
    } catch {
      setSubmitError('Erreur réseau pendant l’inscription.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (loading) {
    return (
      <main className="mx-auto w-full max-w-7xl px-6 py-10 lg:px-10">
        <p className="text-sm text-zinc-500">Chargement de l’inscription...</p>
      </main>
    );
  }
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_330px]">
        ...
        <aside className="rounded-2xl border border-black/10 bg-white p-5 dark:border-white/15 dark:bg-zinc-900">
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-3">
            ...
            >
              {submitting ? 'Inscription...' : 'S’inscrire maintenant'}
            </button>
          </form>
        </aside>
      </section>
