...
export default function SportEvenementRecuPage() {
  ...
  useTrackReceiptView({
    module: 'SPORT_EVENEMENT',
    receiptId: orderId,
    entityId: eventId ?? null,
    receiptType: 'SPORT_EVENT_RECEIPT',
  });

  const verificationPayload = useMemo(
    () =>
      JSON.stringify({
        type: 'SPORT_EVENT_RECEIPT',
        orderId,
        eventId,
        eventName,
        fullName,
  ...
  function handleExportTxt() {
    const lines = [
      'RECU EVENEMENT SPORTIF',
      `Reference inscription: ${orderId}`,
      `Evenement: ${eventName}`,
      `Client: ${fullName}`,
    ...
    anchor.href = url;
    anchor.download = `recu-evenement-sport-${orderId}.txt`;
    anchor.click();
