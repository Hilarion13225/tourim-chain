...
export default function SportActivitePaiementPage() {
  ...
  async function handleConfirmScan() {
    ...
    router.push(
      `/sport/activite/${activityId}/recu?${receiptParams.toString()}`,
    );
  }
    ...
    <main className="mx-auto w-full max-w-5xl space-y-6 px-6 py-8 lg:px-10">
      ...
      <section className="space-y-4 rounded-3xl bg-sky-500 p-6 text-white">
        <div className="space-y-1">
          <p className="text-4xl font-extrabold">Paiement activité sportive</p>
          <p className="text-sm text-sky-50">
            {paymentMethodDisplay} • Réservation #{orderId}
          </p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => router.push(`/sport/activite/${activityId}`)}
